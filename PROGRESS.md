...
| 2026-07-24 | Antigravity | 종목 상세 페이지 상단 히어로 타일의 '20일 하락 확률' 뱃지 우측 '안전 🟢 / 주의 🔴' 아이콘 태그를 깔끔하게 완전 삭제하여 헤더 UI를 간결화 완료 |
...
| 2026-07-23 | Antigravity | 종목 상세 페이지 모던 2컬럼 스마트 대시보드 레이아웃 개편: 좌측 8컬럼(히어로 타일·인터랙티브 차트·AI 브리핑·위험근거) + 우측 4컬럼(XGBoost 하락확률·ESG 3대 지표·자산 영향도 계산기) 배치 완료 |
| 2026-07-23 | Antigravity | 종목 상세 우측 사이드바 최상단에 '내 자산 실시간 영향도 계산기' 돋보이게 재배치 및 주가 하락률(-1% ~ -20%) 인터랙티브 슬라이더 체감 기능 적용 |
| 2026-07-23 | Antigravity | 종목 상세 페이지 100% 풀 화면(max-w-[1920px]) 확장 및 3대 고도화: 4분할 KPI 요약 뱃지 타일, 그리드 가이드 라인 탑재 대형 인터랙티브 차트(h-36), 개미 펫 스마트 종목 코멘트 알약 뱃지 구현 |
| 2026-07-23 | Antigravity | 종목 상세 중복 요소 스마트 통합 & 가독성 극대화: 단일 통합 헬스체크 히어로 타일(날씨·시총·52주·하락확률 뱃지) 및 개미 펫 AI 통합 리포트 카드 단일화 완료 |
| 2026-07-23 | Antigravity | 종목 상세 페이지 백엔드 데이터베이스(/risk-score, /api/dashboard-weather, /risk-evidences) 실시간 주가·등락률·AI 브리핑 동적 API 연동 완료 |
| 2026-07-23 | Antigravity | SK하이닉스(000660) 실제 한국거래소 최신 시장가(268,500원, 시총 195.4조, 52주 308,500원) 및 주요 종목 시장가 실시간 동기화 업데이트 |
| 2026-07-24 | Antigravity | 메인 대시보드 관심 주식(WatchlistCard)의 NAVER(035420) 폴백 주가(182,000원)를 종목 상세페이지 기준 실시간 시장가(169,500원)와 100% 동일하게 동기화 교정 완료 |
...
| 2026-07-23 | Antigravity | AI 진단 리포트 문구 및 팝업 모달 설명을 어려운 통계 전문 용어 대신 친숙하고 간결한 개미 맞춤형 평어로 전체 텍스트 다듬기 완결 |
| 2026-07-23 | Antigravity | SK하이닉스(000660) 폴백 하락확률 6.2% 동기화, [AI 진단] 태그 제거 및 개미의 우산 AI 종합 리포트 타이틀 단일 우산 로고 아이콘 교체 완료 |
| 2026-07-24 | Antigravity | 20개 전체 포트폴리오 종목 상가/리포트 완벽 연동(LG화학, LG, 엔씨소프트, LG엔솔, 삼성SDI, 에코프로 등 추가) 및 보유 자산 차트(AssetChart) 클릭 연동 라우팅 보완 완료 |
| 2026-07-24 | Antigravity | 차트 하단 최고가(좌측 강조)/최저가(우측) 위치 직관화 및 기간별(1D:12pt, 1W:7pt, 1M:30pt, 1Y:52pt) 동적 실제 종가 데이터 시리즈 반영 완결 |
| 2026-07-24 | Antigravity | 주가 차트 노드 빽빽함 최적화: 기간별 시각 노드 수 절제(5~10개) 및 마우스 호버 중심 매끈한 세련된 SVG 곡선으로 시각 디자인 개선 |
| 2026-07-24 | Antigravity | 종목 상세 페이지 카드 순서 재배치: [인터랙티브 차트 ➔ 핵심 위험 근거 ➔ 개미의 우산 AI 종합 리포트] 흐름으로 정보 전달력 강화 |
| 2026-07-24 | Antigravity | 주가 차트 꺾은선 곡선의 시점/종점 위치와 100% 일치하도록 최고가/최저가 위치 정밀 보정: 상승 그래프(📈)는 좌측 최저 ➔ 우측 최고(🟢), 하락 그래프(📉)는 좌측 최고 ➔ 우측 최저(🔴) |
| 2026-07-24 | Antigravity | 상세페이지 주가 차트 좌표 연산 변수 누락으로 인한 런타임 오류(`ReferenceError`) 수정 완료 |
...
| 2026-07-25 | Antigravity | 독립된 투자성향 진단 결과 리포트 컴포넌트(DiagnosisResultView) 신설, /diagnosis/result 및 /onboarding/result 라우트 고도화, 사이드바 헤더 메뉴('성향 종합 리포트') 연결 완료 |
| 2026-07-25 | Antigravity | 투자성향 종합 리포트 뷰를 모바일/단일열 스타일에서 와이드 데스크톱 웹 대시보드 2컬럼 레이아웃(8:4 비대칭 대시보드 구조, max-w-7xl)으로 전면 개편 완료 |
| 2026-07-25 | Antigravity | 투자성향 종합 리포트에 성향별(보수/중립/성장/공격) 맞춤 추천 대표 포트폴리오 종목 4선 및 권장 비중, 특성 태그, 추천 이유 카드 그리드 신규 추가 완료 |
| 2026-07-25 | Antigravity | 추천 포트폴리오 카드 우측 상단에 인터랙티브 '🔄 다른 종목 조합 보기' 새로고침/셔플 기능 및 모션 애니메이션 구현 완료 |
| 2026-07-25 | Antigravity | 추상적이었던 '목표 위험 감내 지수(0~100)' 수치 카드를 완전 제거하고, 직관적인 서비스 정체성 카드인 '☔ 내 맞춤 우산 쉴드 카드로 전면 교체 완료 |
| 2026-07-25 | Antigravity | '☔ 내 맞춤 우산 쉴드' 카드를 사용자의 위기 반응 프로세스에 맞춰 '📰 1단계 (나쁜 소식 감지)', '🚨 2단계 (급락 확률 경보)', '🛡️ 3단계 (내 대응 수칙)'의 또박또박 명확한 3단계 작동 규칙 카드로 최종 교체 완료 |
| 2026-07-25 | Antigravity | 추천 포트폴리오 항목에 3번째 다채로운 대표 추천 종목 조합(총 3개 세트)을 신규 추가하여 새로고침 버튼 순환(조합 1/3 ➔ 2/3 ➔ 3/3) 풍성함 강화 완료 |
| 2026-07-25 | Antigravity | 투자성향 리포트 상단에 5가지 투자성향(보수/안정추구/중립/성장/공격) 라이브 스위처 토글 탭을 신규 구현하여 성향별 캐릭터, 자산비중, 우산 쉴드 규칙, 추천 종목이 동적 갱신되도록 고도화 완료 |
| 2026-07-25 | Antigravity | 성향 비교 토글 스위치 탭을 메인 진단 셀 카드(히어로 브리핑) 내부 상단에 슬림하고 세련된 컴팩트 인라인 알약 뱃지(Compact Inline Pills) 스타일로 전면 리디자인 배치 완료 |
| 2026-07-25 | Antigravity | 토글 스위처의 군더더기 외곽선과 텍스트를 제거하고 극도로 심플한 미니 세그먼트 알약 탭(`🛡️ 보수 | 🦉 안정 | ⚖️ 중립 | ⛵ 성장 | 🔥 공격`)으로 미니멀리즘 전면 리디자인 완료 |
| 2026-07-25 | Antigravity | 미니 토글 스위치 미선택 배경 및 래퍼 색상을 튀는 이질적 회색(Slate) 대신 메인 진단 셀의 은은한 에메랄드/다크 톤과 완전 동화되도록 톤앤매너 컬러 튜닝 완료 |
| 2026-07-25 | Antigravity | 성향 비교 토글 전환 시 '성향 분석 종합 브리핑'이 단순 1줄로 중복 노출되던 버그를 수정하여 5가지 성향별(보수/안정추구/중립/성장/공격) 풍성한 고유 종합 브리핑 리포트 텍스트(fullDescription)가 동적으로 교체되도록 보완 완료 |
| 2026-07-25 | Antigravity | 보수형(에메랄드 그린 `#10b981`)과 안정추구형의 컬러가 비슷했던 디자인을 개선하여, 안정추구형 시그니처 시각 컬러를 맑고 청량한 시안/청록(`cyan-500` / `#06b6d4`)으로 선명히 차별화 완료 |
| 2026-07-25 | Antigravity | 보수형(`CONSERVATIVE`)과 위험중립형(`BALANCED`)의 시그니처 테마 컬러를 교체하여 보수형=블루(`blue-500`), 위험중립형=에메랄드 그린(`emerald-500`)으로 재배치 완료 |
| 2026-07-25 | Antigravity | 성장형(`GROWTH`)과 안정추구형(`MODERATE_CONSERVATIVE`)의 시그니처 테마 컬러를 교체하여 성장형=시안/청록(`cyan-500`), 안정추구형=엠버 오렌지(`amber-500`)로 재배치 완료 |
| 2026-07-25 | Antigravity | 안정추구형(`MODERATE_CONSERVATIVE`)과 위험중립형(`BALANCED`)의 시그니처 테마 컬러를 교체하여 안정추구형=에메랄드 그린(`emerald-500`), 위험중립형=엠버 오렌지(`amber-500`)로 최종 교체 완료 |
| 2026-07-25 | Antigravity | 우산 쉴드 카드 우측 상단 '실시간 방어망 ON/OFF' 클릭 인터랙티브 스위치 기능 및 OFF 시 딤(grayscale) 처리 및 경고 안내 UI 구현 완료 |
| 2026-07-25 | Antigravity | 투박했던 방어망 뱃지를 지우고 극도로 깔끔하고 세련된 미니 iOS 슬라이더 토글 스위치(방어망 ON/OFF 🔘)로 미니멀리즘 전면 개편 완료 |
| 2026-07-25 | Antigravity | 상단 GNB 내비게이션 바와 메뉴가 중복되는 불필요한 'Quick Action' 카드를 깔끔히 완전 제거하여 우측 레이아웃 세련미 및 화면 여백 가독성 극대화 완료 |
| 2026-07-25 | Antigravity | 일반 증권사의 천편일률적 분산표시였던 '권장 자산 배분 비중' 카드를 제거하여 ESG 악재 감지 및 XGBoost 하락 확률 예측이라는 서비스 본연의 핵심 정보에 집중된 슬림 리포트로 군더더기 전면 정리 완료 |
| 2026-07-25 | Antigravity | 우측 '☔ 우산 쉴드 3단계 작동 규칙' 카드와 내용이 중복되고 전문용어로 혼동을 유발하던 'ESG & 마켓 위험 대응 가이드라인' 카드를 전면 삭제하여 최고 수준의 직관적 다이어트 완료 |
| 2026-07-25 | Antigravity | 추천 종목 카드 하단에 '🎯 이 추천 종목 조합으로 내 포트폴리오 기상도 모니터링 시작하기' 원클릭 연동 대형 버튼 및 우측 '🛡️ 위기 탈출 멘탈 케어 팁' 카드 구현 완료 |
| 2026-07-25 | Antigravity | 우측 사이드바 레이아웃 순서를 재배치하여 '🛡️ 위기 탈출 멘탈 케어 팁' 카드를 맨 하단으로 내려 시각적 안정감 극대화 완료 |
| 2026-07-25 | Antigravity | '2단계 급락 확률 경보(하락 확률 > X%)'의 AI 예측 확률 임계치와 '🚨 하락 경보 모니터링 기준선' 카드의 뱃지 및 설명 문구를 100% 명확히 일치하도록 논리적 정합성 교정 완료 |
...
| 2026-07-25 | Antigravity | 중복/분산되어 있던 '하락 경보 모니터링 기준선' 카드를 '우산 쉴드 2단계(AI 경보 삐삐)' 내부에 미니 뱃지 및 설명 텍스트 형태로 쏙 흡수 통합 완료 |
| 2026-07-25 | Antigravity | 메인 화면 보유 자산 탭을 '주식 보유량' 카드로 개편하고, 실시간 현재가와 평단가를 연동한 수익률(%), 보유 액수 및 수익(손실) 액수 실시간 계산/표시 기능 추가 및 하단 개별 종목 자산 구성 목록에 보유량/평단가/수익률/비중 통합 렌더링 구현 완료 |
