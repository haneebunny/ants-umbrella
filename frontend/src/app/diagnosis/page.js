...
import { DEMO_PROFILE, PORTFOLIO_PRESETS } from '../data/mockData';
...
export default function DiagnosisPage() {
  ...
  const currentPreset = PORTFOLIO_PRESETS.find(p => p.id === selectedId) || PORTFOLIO_PRESETS[0];
  ...
  const baseRisk = currentPreset?.riskRadar?.currentPortfolioRisk || 45;
      ...
      <main className="pt-2 pb-10 px-1 max-w-4xl">
        ...
        <div className={`rounded-2xl border p-6 space-y-6 ${isDark ? 'bg-[#1e2220] border-white/5' : 'bg-white border-slate-100 shadow-sm'}`}>
          ...
          <div className="space-y-4 pt-2">
            ...
            <div>
              <div className="flex justify-between text-xs mb-1.5">
                <span className={isDark ? 'text-slate-400' : 'text-slate-500'}>시뮬레이션 포트폴리오 위험</span>
