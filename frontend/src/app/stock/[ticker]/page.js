...
// 종목별 상세 Mock 데이터
const STOCK_DATA = {
  '005930': { name: '삼성전자', marketCap: '458.2조', high52w: '89,000원', low52w: '62,000원', currentPrice: '76,800원', weather: 'sunny', direction: 'up', confidence: 'strong', change: 1.5, weight: 25, dropProb: 8.4,
  ...
  '000660': { name: 'SK하이닉스', marketCap: '137.9조', high52w: '240,000원', low52w: '115,000원', currentPrice: '189,500원', weather: 'sunny', direction: 'up', confidence: 'strong', change: 1.8, weight: 22, dropProb: 6.2,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
          ...
          <div className="lg:col-span-8 space-y-4">
            ...
            <div className={`rounded-2xl border p-5 sm:p-6 ${isDark ? 'bg-[#1e2220] border-white/5' : 'bg-white border-slate-100 shadow-sm'}`}>
              ...
              <div className="flex justify-between mt-3 pt-2.5 border-t border-white/5 text-xs">
                <span className={`font-mono ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                  최저 {minP.toLocaleString()}원
                </span>
                <span className={`font-mono font-black ${isUp ? (isDark ? 'text-[#69dbad]' : 'text-[#3eb489]') : 'text-rose-500'}`}>
              ...
              <div className={`rounded-2xl border p-5 sm:p-6 ${isDark ? 'bg-[#1e2220] border-white/5' : 'bg-white border-slate-100 shadow-sm'}`}>
                ...
                <div className="space-y-3">
                  {stock.evidences.map((ev, i) => (
                    <div key={i} className={`flex items-start justify-between gap-3 p-4 rounded-xl transition-all ${isDark ? 'bg-white/5 hover:bg-white/10' : 'bg-slate-50 hover:bg-slate-100'}`}>
                      ...
                      <span className={`text-xs font-mono flex-shrink-0 mt-0.5 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                        {ev.date || '2026.07.23'}
                      </span>
                    </div>
                  ))}
