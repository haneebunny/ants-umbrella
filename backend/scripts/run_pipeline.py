...
def run_step(script_name: str, description: str, log_file) -> bool:
    ...
    for line in process.stdout:
        ...
        safe_line = line.encode(term_encoding, errors='replace').decode(term_encoding)
    ...
    if process.returncode == 0:
        print(f"[성공] {description} 완료")
        return True
    else:
        print(f"[오류] {description} 실패 (Exit Code: {process.returncode})")
        return False
...
def main():
    ...
    print("==================================================")
    print("ESG 투자성향진단 통합 ML 파이프라인 구동 스크립트")
    ...
    with open(log_path, "w", encoding="utf-8") as log_file:
        ...
        for script_name, description in PIPELINE_STEPS:
            ...
            if not step_success:
                print(f"\n[실행 중단] {description} 단계에서 오류가 발생하여 파이프라인을 중단합니다.")
                success = False
                break
