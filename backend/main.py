...
def generate_ai_briefing(ticker_name: str, ticker: str, prob_up: float, direction: str, confidence_tier: str, esg_count: int = 0) -> str:
    ...
    # ── 4. Gemini로 신규 브리핑 생성 ──────────────────────────────────
    prompt = f"""당신은 주식 초보자에게 친근하게 정보를 전달하는 '미미' 캐릭터입니다.
...
def get_weather_briefing(body: dict):
    ...
    TONE_INSTRUCTION = {
        "thunder": (
            "⚠️ 현재 날씨는 '번개(위험)'입니다. "
            "반드시 긴급경고 톤으로 작성하세요. "
            "포트폴리오에 심각한 위험이 있음을 경고하고, 즉각적인 대응(비중 축소·손절 기준 확인 등)을 권고해야 합니다. "
            "절대 긍정적이거나 안심하는 표현을 사용하지 마세요."
        ),
        "rainy": (
            "⚠️ 현재 날씨는 '비(주의)'입니다. "
            "주의·경계 톤으로 작성하세요. "
            "하락 리스크가 감지되므로 비중 조절·리밸런싱을 권고하는 내용을 담아야 합니다. "
            "안심하거나 긍정적인 표현은 사용하지 마세요."
        ),
        "cloudy": (
            "현재 날씨는 '구름(보통)'입니다. "
            "중립적 톤으로 작성하세요. "
