...
| 2026-07-27 | `frontend/src/app/diagnosis/weather/page.js` | 모바일에서 종목별 위험 요약의 상승/하락 텍스트 위치 이상 수정: 왼쪽 종목 영역에 `min-w-0` 추가, 오른쪽 상승/하락 영역에 `flex-shrink-0 ml-2 whitespace-nowrap` 추가로 텍스트가 밀리거나 줄바꿈되는 현상 방지. 추가로 `py-0.2`(유효하지 않은 Tailwind 값) → `py-0.5`로 교정. |
