...
| 2026-07-25 | Antigravity | 위험 알림 페이지(/api/alerts) 에서 긍정(positive) 방향 뉴스를 알림 목록에서 제외하도록 필터링 추가. 알림 제목은 실제 기사 제목 대신 ESG 카테고리 기반 범용 표현(예: 'ESG 환경(E) 미디어 이슈 포착')을 유지. 최대 15건 제한 로직도 필터링 이후 적용되도록 수정 |
